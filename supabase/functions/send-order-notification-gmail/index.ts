...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    console.log("Sending Gmail order notification for:", orderNumber);
    ...
    // Send email using Gmail SMTP via a webhook service
    const emailData = {
      from: "VilĀura Orders <vilaura.orders@gmail.com>",
    ...
    // Use a reliable email service (EmailJS or similar)
    try {
      // For now, let's use a simple webhook approach
      ...
      const emailjsData = {
        ...
        template_params: {
          ...
          order_items: items.map(item => `${item.name} x${item.quantity} = ₹${(item.price * item.quantity).toFixed(2)}`).join(', ')
        }
      };
      ...
      // For immediate testing, let's also log to console
    ...
    console.error("Error in Gmail send-order-notification function:", error);
    return new Response(
